...
<html>
    ...
    <body>
        ...
        <h4 align="center"><a href="Graph.jsp" style="color:red;font-weight: bold;font-family: cursive;">Click to View Result in Graph</a></h4>
        <%
            ...
            query=con.prepareStatement("truncate table general"); 
            rs=query.executeQuery();
            
            query=con.prepareStatement("select * from recommend"); 
            rs=query.executeQuery();
            String ReviewsTemp="";
            while(rs.next())
            {
                ...
                if(P)
                {
                    PreparedStatement c=con.prepareStatement("insert into positive_reviews(product,reviews) values('"+Product+"','"+Reviews+"') ");
                    c.executeUpdate();
                }
                //Inserting Negative Reviews
                else if(N)
                {
                    PreparedStatement c=con.prepareStatement("insert into negative_reviews(product,reviews) values('"+Product+"','"+Reviews+"') ");
                    c.executeUpdate();
                }
                //Inserting Nuetral Reviews
                else if(Nt)
                {
                    PreparedStatement c=con.prepareStatement("insert into general(product,reviews) values('"+Product+"','"+Reviews+"') ");
                    c.executeUpdate();
                }
            }//While Loop Ending
...
<!--View Ranking Classification-->
        <%
            ...
            while(rs.next())
            {
                ...
                {
                    PreparedStatement geting=con.prepareStatement("SELECT COUNT(product)Total FROM general WHERE product='"+rs.getString("product")+"' ");
                    ResultSet gets=geting.executeQuery();
                    if(gets.next())
                    {
